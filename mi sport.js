...
class MiMotion:
    ...
    # 获取时间方法1：公共时间戳服务（需注意服务的稳定性）
    # def get_time(self):
    #     url = "http://worldtimeapi.org/api/timezone/Asia/Shanghai"
    #     res = requests.get(url, headers=self.headers).json()
    #     t = str(res['unixtime'])+'000'
    #     return t
    
    # 获取时间方法2：直接使用 Python 的 time 模块获取当前时间戳，而无需调用外部 API
    def get_time(self):
    ...
    def main(self):
        ...
        for check_item in self.check_items:
            ...
            login_token, user_id = self.login(phone, password)
            ...
            if login_token == 0:
                ...
                data_json = re.sub(
                    find_step.findall(data_json)[0], step, str(data_json)
                )
...
def string_to_dict(s):
    ...
    result_dict['min_step'] = int(parts[2])
    result_dict['max_step'] = int(parts[3])
    return result_dict
